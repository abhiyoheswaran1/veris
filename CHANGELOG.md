...
- Provable verification. `veris attest` packages the latest `veris verify` into a portable, Ed25519-signed in-toto attestation of the exact commit; `veris gate` blocks unless a valid attestation meets `.veris/policy.json` (integrity of the record, freshness against HEAD with a clean tree, the required verdict, required capabilities×languages, and — when `require.signers` is set — a trusted signer). Without `require.signers` a gate is integrity-only and does not establish authorship; set signers and sign (`VERISKIT_SIGNING_KEY`/`--key`) for a real trust gate. No new dependencies. Keyless/Sigstore signing is planned.
